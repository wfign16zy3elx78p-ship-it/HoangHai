...
function buildEmailHtml(b: BookingPayload): string {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/><style>
    ...
    .card{background:#fff;max-width:520px;margin:0 auto;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08)}
    ...
    .header p{margin:6px 0 0;font-size:12px;color:rgba(255,255,255,.45)}
    ...
    td:first-child{color:#999;text-transform:uppercase;font-size:11px;letter-spacing:.8px;width:38%}
    ...
    <div class="header"><h1>BYD — New Test Drive</h1><p>Booking notification</p></div>
    <div class="body">
      ...
      ${b.notes ? `<div style="margin-top:16px;font-size:12px;color:#888;background:#fafafa;padding:10px 14px;border-left:3px solid #ddd"><strong>Notes:</strong> ${b.notes}</div>` : ""}
    </div>
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  await Promise.allSettled([
    sendTelegram(buildTelegramText(b))
      .then(() => { results.telegram = "sent"; })
      .catch((err) => { results.telegram = `failed: ${String(err.message)}`; }),
    sendEmail(
      `🚗 New Test Drive — ${b.modelName} — ${b.firstName} ${b.lastName}`,
      buildEmailHtml(b)
    )
      .then(() => { results.email = "sent"; })
      .catch((err) => { results.email = `failed: ${String(err.message)}`; }),
  ]);
